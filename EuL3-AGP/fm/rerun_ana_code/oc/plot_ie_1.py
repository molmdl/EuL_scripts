...
def plot_heatmap(mean_IE, resids, resnames, bins, d1_min, d1_max, out_path, title_prefix="", neg_cutoff=-40.0, pos_cutoff=10.0, plot_labels=True):
    ...
    # Ensure vmin < 0 for TwoSlopeNorm (vcenter=0)
    vmin_norm = vmin if vmin < 0 else -0.1
    norm = TwoSlopeNorm(vcenter=0, vmin=vmin_norm, vmax=vmax)
    ...
    neg_ticks = np.linspace(vmin_norm, 0, 5)
    ...
    with warnings.catch_warnings():
        ...
        for idx, res in enumerate(resids):
            ...
            if not np.isnan(min_val) and min_val < neg_cutoff:
                ...
                labels_to_plot.append((peak_d1, res, f"{rname}{res}", min_val))
            ...
            if not np.isnan(max_val) and max_val > pos_cutoff:
                ...
                labels_to_plot.append((peak_d1, res, f"{rname}{res}", max_val))
    ...
    if csv_rows:
        ...
        if plot_labels:
            ...
            for peak_d1, res, text, val in labels_to_plot:
                color_star = 'yellow' if val < 0 else 'black'
                plt.plot(peak_d1, res, '*', color=color_star, markersize=10, 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--label_neg', type=float, default=-40.0, help='Negative IE cutoff to label major contribution (kJ/mol)')
    parser.add_argument('--label_pos', type=float, default=10.0, help='Positive IE cutoff to label major contribution (kJ/mol)')
    ...
    for rep in args.replicas:
        ...
        if mean_IE is not None:
            ...
            plot_heatmap(mean_IE, resids, resnames, bins_arr, args.d1_min, args.d1_max, out_png, 
                         ...
                         plot_labels=not args.no_plot_labels)
