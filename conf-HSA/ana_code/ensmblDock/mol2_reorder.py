...
def collect_sdf_files(sdf_patterns):
    """
    Collect SDF file paths from input patterns.

    Handles:
    - Explicit file paths (verified to exist)
    - Glob patterns (shell-expanded before reaching Python)
    - Mixed lists

    Parameters
    ----------
    sdf_patterns : list of str
        File paths or patterns from command line

    Returns
    -------
    list of str
        Verified existing file paths (sorted for reproducibility)
    ...
    if missing:
        raise FileNotFoundError(
            f"SDF file(s) not found: {', '.join(missing)}"
        )
...
def select_best_across_sdfiles(sdf_paths, metric=DEFAULT_METRIC):
    """
    Find the best model across multiple SDF files.

    Parameters
    ----------
    sdf_paths : list of str
        Paths to SDF files to search
    metric : str
        Score field to optimize

    Returns
    -------
    best_model : dict
        The selected model dict (same format as from parse_sdf)
    source_file : str
        Path to the SDF file containing the best model
    model_num : int
        Model number within that file (1-based)
    """
    best_model = None
    best_score = None
    best_file = None
...
def list_models_across_sdfiles(sdf_paths, metrics=None):
    """
    Print a table of all models from multiple SDF files with their scores.

    Each row includes the source filename for attribution.

    Parameters
    ----------
    sdf_paths : list of str
        Paths to SDF files to list
    metrics : list of str, optional
        Score fields to display. If None, shows all available metrics.
    """
    if metrics is None:
        # Collect all score keys from all files
        all_keys = []
        seen = set()
        for sdf_path in sdf_paths:
            models = parse_sdf(sdf_path)
    ...
    # Build header with filename column
    max_fn_len = max(len(os.path.basename(p)) for p in sdf_paths)
    fn_width = max(max_fn_len, 8)  # at least 8 chars for "File" header
...
def build_parser():
    ...
    p_gro.add_argument("-s", "--sdf", required=True, nargs="+", metavar="SDF",
                       help="One or more SDF files. Supports glob patterns via shell "
                            "expansion (e.g., hsa*-dzp.sdf). Multiple files: "
                            "-s file1.sdf file2.sdf")
    p_gro.add_argument("-o", "--output", metavar="GRO",
...
def main():
    ...
    if args.mode == "mol2":
        ...
        # Collect and validate SDF files
        sdf_paths = collect_sdf_files(args.sdf)

        # Handle --list-models first (doesn't require --output)
        if args.list_models:
            if len(sdf_paths) == 1:
                models = parse_sdf(sdf_paths[0])
                list_models(models)
            else:
                list_models_across_sdfiles(sdf_paths)
            return

        # Require --output for actual conversion
        if not args.output:
            parser.error("--output is required unless --list-models is specified.")

        if len(sdf_paths) == 1:
            # Single SDF - use existing flow
            sdf_pose_to_gro(
                itp_path=args.itp,
                sdf_path=sdf_paths[0],
                out_path=args.output,
            ...
            # Multiple SDFs - new flow
            if args.model is not None:
                print("WARNING: --model is ignored when multiple SDF files are provided.",
                      file=sys.stderr)
                print("         Best model will be selected by --metric across all files.",
                      file=sys.stderr)

            best_model, best_file, best_model_num = select_best_across_sdfiles(
                sdf_paths, metric=args.metric
            )
            ...
            sdf_pose_to_gro(
                ...
                mol2_template_path=args.template,
                metric=args.metric,
                model_num=best_model_num,  # Select the specific model in that file
            )
