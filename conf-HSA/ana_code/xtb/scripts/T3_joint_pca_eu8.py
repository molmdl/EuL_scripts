#!/usr/bin/env python3
"""
T3_joint_pca_eu8.py — Parameterized joint PCA on all 16 xTB trajectories.

Supports four PCA variants (A1–A4) via CLI arguments:
  A1/A2 (--tag eu8 / eu8_all):
    Eu+8 alignment [0,1,2,3,4,5,6,7,54], all 68 scaffold heavy atoms (204 feats)
  A3 (--tag eu8_nochrom --exclude-features 102,103,104,105,107,109,113,114,115,116,118,120):
    Eu+8 alignment, exclude Ring C+D distal chromophore (56 atoms, 168 feats)
  A4 (--tag eu8_ringa --align-indices 0,1,2,3,4,5,6,7,54,73,74,75,76,80):
    Eu+8+Ring A alignment (14 atoms), all 68 scaffold heavy atoms (204 feats)
...
Usage:
  python scripts/T3_joint_pca_eu8.py \
      --data-dir data \
      --out-dir analysis \
  ...
  # A3 variant (no Ring C+D):
  python scripts/T3_joint_pca_eu8.py \
      --tag eu8_nochrom \
      --exclude-features 102,103,104,105,107,109,113,114,115,116,118,120

  # A4 variant (Ring A in alignment):
  python scripts/T3_joint_pca_eu8.py \
      --tag eu8_ringa \
      --align-indices 0,1,2,3,4,5,6,7,54,73,74,75,76,80
"""
...
# Default Eu+8 alignment: Eu + 8 donor atoms (cap N63 excluded)
ALIGN_EU8_DEFAULT = sorted([0, 1, 2, 3, 4, 5, 6, 7, 54])
...
def parse_comma_ints(s):
    """Parse a comma-separated string of integers, return sorted list."""
...
def compute_alignment_rmsd(aligned_coords, ref_align_pos, align_indices):
    """
    Compute per-frame RMSD of alignment atoms after alignment.

    Parameters
    ----------
    aligned_coords : ndarray (n_frames, n_atoms, 3)
        Already-aligned coordinates.
    ref_align_pos : ndarray (n_align, 3)
        Reference alignment positions.
    align_indices : list of int
        Alignment atom indices.

    Returns
    -------
    mean_rmsd : float
        Mean RMSD across all frames.
    per_frame_rmsd : ndarray (n_frames,)
...
def compute_alignment_rmsd_subset(aligned_coords, ref_frame, atom_indices):
    """
    Compute per-frame RMSD for a SUBSET of atoms after full alignment.
    This is useful for diagnosing how well specific atom groups were aligned
    (e.g., Ring A atoms in Eu+8+Ring A alignment).
    ...
    atom_indices : list of int
        Subset of atom indices to compute RMSD for.

    Returns
    -------
    mean_rmsd : float
        Mean RMSD across all frames for this subset.
    """
    n_frames = aligned_coords.shape[0]
...
def run_pca(args):
    """
    Execute the parameterized joint PCA pipeline.

    This function contains the core logic and can be called programmatically
    ...
    # ── Parse CLI parameterization ──────────────────────────────
    align_indices = parse_comma_ints(args.align_indices)
    if align_indices is None:
        align_indices = ALIGN_EU8_DEFAULT
    ...
    print("=" * 65)
    print(f"T3 Joint PCA (Eu+8 variant) — tag={tag}")
    print("=" * 65)
    ...
    # ── 4. Identify scaffold heavy atom feature indices ──────────
    print("\n[4/7] Identifying scaffold heavy atoms and feature set ...")
    ...
    print("\n[5/7] Kabsch-aligning all frames to reference "
          "(me_rrrD_sap frame 0) ...")

    # Reference = first system (sorted: me_rrrD_sap), frame 0
    ref_frame = all_systems[0]["coords"][0]       # (n_atoms, 3)
    ref_align_pos = ref_frame[np.array(align_indices)]  # (n_align, 3)

    # Track alignment RMSD diagnostics
    align_rmsd_summary = {}
    ...
    for sys_data in all_systems:
        ...
        # Compute alignment-atom RMSD diagnostic
        ...
        print(f"  Aligned {sys_data['name']} ({n_frames} frames, "
              f"align RMSD={mean_rmsd:.4f} A)")

    # Print alignment RMSD summary
    print(f"\n  Alignment RMSD summary (tag={tag}):")
    for sys_name in sorted(align_rmsd_summary.keys()):
        rmsd = align_rmsd_summary[sys_name]
        print(f"    {sys_name:25s}  {rmsd:.4f} A")

    # If alignment includes Ring A (A4 variant), print split diagnostics
    RING_A_IDX = [73, 74, 75, 76, 80]
    core_9 = ALIGN_EU8_DEFAULT
    if set(RING_A_IDX).issubset(set(align_indices)) and len(align_indices) > len(core_9):
        print(f"\n  Ring A alignment strain diagnostic:")
        # Need ref_frame for subset RMSD computation
        for sys_data in all_systems:
            mean_core, _ = compute_alignment_rmsd_subset(
                sys_data["aligned"], ref_frame, core_9)
            mean_ringa, _ = compute_alignment_rmsd_subset(
                sys_data["aligned"], ref_frame, RING_A_IDX)
            print(f"    {sys_data['name']:25s}  core-9 RMSD={mean_core:.4f} A  "
                  f"Ring-A RMSD={mean_ringa:.4f} A  "
                  f"ratio={mean_ringa / max(mean_core, 1e-10):.1f}x")
    ...
    for sys_data in all_systems:
        # Feature atoms from aligned coords
        feat_coords = sys_data["aligned"][:, feature_idx_arr, :]  # (n_frames, n_feat, 3)
        features = feat_coords.reshape(sys_data["n_frames"], -1)  # (n_frames, n_feat * 3)
    ...
    # ── Loadings CSV ─────────────────────────────────────────────
    # pca.components_ shape: (10, n_features) -> reshape to (10, n_feat_atoms, 3)
    n_feat_atoms = len(FEATURE_IDX)
    components_3d = pca.components_.reshape(N_PCS, n_feat_atoms, 3)
    # Per-atom loading magnitude per PC
    atom_loading_mag = np.sqrt((components_3d ** 2).sum(axis=2))  # (10, n_feat_atoms)
    ...
    # Clip to visible range (above ~15 kJ/mol is essentially unvisited)
    G = np.clip(G, 0, 15)
    ...
    # Shared capped energy range so all subplots are visible
    G_MAX_CLIP = 12  # kJ/mol — above this is effectively unvisited for 2000 frames
    ...
    for idx, sys_name in enumerate(systems_sorted):
        ...
        cf = ax.contourf(Xs, Ys, Gs.T, levels=levels_shared, cmap="cubehelix",
    ...
    print(f"  Feature matrix:       {X.shape}")
    print(f"  Alignment indices:    {align_indices} ({len(align_indices)} atoms)")
...
def build_parser():
    ...
    parser = argparse.ArgumentParser(
        description="Parameterized joint PCA on all 16 xTB trajectories. "
                    "Supports Eu+8 alignment variants A1-A4 via CLI args.")
    ...
    parser.add_argument("--align-indices", type=str, default=None,
                        help="Comma-separated SAP atom indices for Kabsch alignment "
                             "(default: Eu+8 = 0,1,2,3,4,5,6,7,54)")
    ...
    parser.add_argument("--tag", type=str, default="eu8",
                        help="Output file tag (e.g., eu8, eu8_all, eu8_nochrom, "
                             "eu8_ringa)")
    return parser
