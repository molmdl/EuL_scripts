...
# Constants
# Gas constant imported from shared module (IUPAC 2019 value)
import sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), 'pca_analysis'))
from pca_utils import R_GAS as R
...
def main():
    ...
    try:
        ...
        # NOTE: Single-pass alignment to PDB reference. For iterative alignment
        # to the mean structure, use combined_pca.py (--n-align-iter 2 default).
        ...
        # Note: uses biased estimator (N). combined_pca.py uses unbiased (N-1)
        # for unweighted PCA. Difference is ~0.02% for N~6000.
        ...
        if T is not None:
            ...
            for traj_idx, (start, end) in enumerate(frame_ranges):
                ...
                for px, py, suffix in pairs:
                    ...
                    # Use shared compute_fes for standard (non-weighted) FES
                    if smooth or weights_traj is not None:
                        # Weighted/smoothed path: keep local implementation
                        ...
                        fe -= fe[np.isfinite(fe)].min()  # Shift so minimum = 0
                    else:
                        from pca_utils import compute_fes
                        fe, xedges, yedges = compute_fes(
                            proj_traj[:, px], proj_traj[:, py],
                            n_bins=BINS, kT=kT,
